extends Node
const OUT := "res://output/chapter4_environments/motion_frames"
func _ready() -> void:
	...
	for stage in range(9):
		...
		env.size=get_viewport().get_visible_rect().size; add_child(env); env.setup(stage)
		...
		var label=Label.new()
		label.text="CHAPTER 4  /  %02d  /  %s"%[stage+1,env.spec.title]
		label.position=Vector2(12,env.size.y-30)
		label.add_theme_color_override("font_outline_color",Color.BLACK)
		label.add_theme_constant_override("outline_size",5)
		add_child(label)
		for frame in range(90 if stage==8 else 45):
			if frame==15:
		...
		await get_tree().process_frame
	print("PALACE MOTION frames=",frame_id)
	get_tree().quit()
