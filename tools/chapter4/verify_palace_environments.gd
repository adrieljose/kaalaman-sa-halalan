...
func _ready() -> void:
	...
	for resolution in [Vector2i(1152,648),Vector2i(390,844)]:
		...
		for who in ["male","female"]:
			for index in range(9):
				...
				check(battle._palace_environment.phase_mix==0.0,"Retry retained boss phase")
				var final_ref=weakref(battle._palace_environment)
				battle.queue_free()
				await frames(3)
				check(final_ref.get_ref()==null,"Scene exit leaked environment")
				print("PALACE verified ",resolution," ",who," stage ",index+1)
	...
	print("PALACE ","PASS" if faults.is_empty() else "FAIL"," checks=",checks)
