...
ROOT=Path(__file__).resolve().parents[2]; OUT=ROOT/'output/chapter4_skill_icons'
def gameplay(text):
...
for row in manifest:
    ...
    if not box or min(box[0],box[1])<2 or max(box[2],box[3])>62: faults.append(row['key']+': unsafe padding')
