extends Node
## Loads Chapter 4 the way the game does and checks every encounter is whole.
##
...
##
##   godot --path . tools/chapter4/probe_chapter4.tscn
...
func _ready() -> void:
	...
	if not GameState.load_chapter(4):
		print("  FAULT  chapter 4 will not load at all")
		get_tree().quit(1)
		return
	...
	for index in chapter.encounter_count():
		...
		for move in enemy.moves:
			...
			if not move.signature_id().begins_with("c4combat_"):
				_faults.append("%s: %s has id %s, expected a c4combat_ id"
	...
	print("")
	if _faults.is_empty():
		print("chapter 4 OK -- 9 encounters, 27 skills, every asset present")
		get_tree().quit(0)
		return
