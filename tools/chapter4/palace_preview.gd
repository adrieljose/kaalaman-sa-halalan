...
func _show_stage() -> void:
	...
	environment=preload("res://scripts/environments/palace_environment.gd").new()
	add_child(environment)
	move_child(environment,0)
	environment.setup(stage)
	caption.text="%d / 9  %s\n1–9 stage | H heavy hit | B boss phase | Space pause | Escape close"%[stage+1,environment.spec.title]
	_layout()
func _layout() -> void:
